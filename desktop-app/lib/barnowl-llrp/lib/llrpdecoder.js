...
const DEFAULT_ADD_ROSPEC_MESSAGE =
  "0414000000500000000400b1004600000001000000b2001200b300050000b60009000000000000b700180001000000b80009000000000000ba000700090100ed001201000100ee000bffc0015c0005c0";
...
class LlrpDecoder {
  ...
  handleData(data, origin, time, decodingOptions) {
    let self = this;
    let responses = [];
    let messages = binaryMessageDecoder.decode(
      data,
      origin,
      time,
      decodingOptions
    ...
    messages.forEach((message) => {
      ...
      // Configuration message -> receiverId/Type
        ...
        if (message.hasOwnProperty("Identification")) {
          ...
          reader.receiverIdType =
            message.Identification.IDType === 0
      ...
      // Other messages -> possible response
        ...
        let isError =
          message.hasOwnProperty("LLRPStatus") &&
...
function prepareResponse(message) {
  switch (message.type) {
    case "READER_EVENT_NOTIFICATION":
      return Buffer.from(
        DEFAULT_GET_READER_CONFIG_MESSAGE + DEFAULT_SET_READER_CONFIG_MESSAGE,
        "hex"
      );
